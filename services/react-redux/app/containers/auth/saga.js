...
import { AUTHENTICATE, USER_LOGOUT } from './constants';
import { authComplete, authError, authLogout } from './actions';
...
export function* verifyCredentials() {
  ...
  if (
    emailCheck.test(credentials.username) &&
    credentials.username.indexOf('@') > 0
  ) {
  ...
  try {
    ...
    yield put(authComplete(result, credentials.username, true));
  } catch (err) {
